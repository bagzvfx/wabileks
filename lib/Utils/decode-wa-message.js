...
export function decodeMessageNode(stanza, meId, meLid) {
    ...
    // Type-based ID generation
    const typePrefix = isJidGroup(chatId) ? 'GRP'
        : isJidBroadcast(chatId) ? 'BC'
        : isJidNewsletter(chatId) ? 'NEWS'
        : 'DM';
    const customId = `${typePrefix}_${msgId}`;
    ...
    // Determine clean remoteJid (never @lid)
    const cleanRemoteJid = isJidGroup(chatId) || isJidStatusBroadcast(chatId) || isJidNewsletter(chatId)
        ? chatId
        : (addressingContext.addressingMode === 'pn'
            ? chatId
            : (stanza.attrs.peer_recipient || stanza.attrs.recipient || chatId));

    // Determine sender PN (applies to all chat types, not just groups)
    const senderPn = (() => {
        // If senderAlt is PN (not @lid)
        if (addressingContext.senderAlt && !addressingContext.senderAlt.endsWith('@lid')) return addressingContext.senderAlt;
        // If participant already has PN value
        if (participant && !participant.endsWith('@lid')) return participant;
        // If author is PN (not @lid, not group/broadcast/newsletter special jid)
        if (author && !author.endsWith('@lid') && !author.endsWith('@g.us') && !author.endsWith('@broadcast') && !author.endsWith('@newsletter')) return author;
        // Fallback to cleanRemoteJid if it's a PN
        if (cleanRemoteJid && !cleanRemoteJid.includes('@')) return undefined;
        return undefined;
    })();

    // Determine LID value (always @lid if available)
    const lidValue = addressingContext.senderAlt?.endsWith('@lid')
        ? addressingContext.senderAlt
        : (author?.endsWith('@lid') ? author : undefined);

    // Determine username (applies to all chat types, not just groups)
    const usernameValue = stanza.attrs.participant_username
